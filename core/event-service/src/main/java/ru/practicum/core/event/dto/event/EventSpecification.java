    ...
    public static Specification<Event> initiator(List<Long> initiatorIds) {
        return (root, query, cb) ->
                (initiatorIds == null || initiatorIds.isEmpty())
                        ? null
                        : root.get("initiator").in(initiatorIds);
    }

    public static Specification<Event> initiator(Long initiatorId) {
        return (root, query, cb) ->
                initiatorId == null ? null : cb.equal(root.get("initiator"), initiatorId);
    }
